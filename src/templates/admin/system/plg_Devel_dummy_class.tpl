...
<form name="form1" id="form1" method="post">
    ...
    <table class="form">
        ...
        <tr>
            <th>各規格毎の分類件数<span class="attention"> *</span></th>
            <td>
                <!--{if $form.min_category.error}--><span class="attention"><!--{$form.min_category.error}--></span><!--{/if}-->
                <!--{if $form.max_category.error}--><span class="attention"><!--{$form.max_category.error}--></span><!--{/if}-->
                <input type="text" name="min_category" value="<!--{$form.min_category.value|h}-->" maxlength="<!--{$form.min_category.maxlength|h}-->" size="6" class="box10" <!--{if $form.min_category.error}--><!--{sfSetErrorStyle}--><!--{/if}--> />
                ～
                <input type="text" name="max_category" value="<!--{$form.max_category.value|h}-->" maxlength="<!--{$form.max_category.maxlength|h}-->" size="6" class="box10" <!--{if $form.max_category.error}--><!--{sfSetErrorStyle}--><!--{/if}--> />
            </td>
        </tr>
        <tr>
            <th>規格名のランダム文字列の長さ<span class="attention"> *</span></th>
            <td>
